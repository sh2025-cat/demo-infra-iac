...
| 권한 | 설명 |
|------|------|
| `ecs:DescribeTasks`, `ecs:ListTasks` | Task 조회 |
| `ecs:RunTask`, `ecs:StopTask` | Task 실행/중지 |
| `ecs:CreateService`, `ecs:UpdateService`, `ecs:DeleteService` | Service 생성/수정/삭제 |
| `ecs:DescribeServices`, `ecs:ListServices` | Service 조회 |
| `ecs:DescribeClusters`, `ecs:ListClusters` | Cluster 정보 조회 |
| `ecs:RegisterTaskDefinition`, `ecs:DeregisterTaskDefinition` | Task Definition 등록/해제 |
| `ecs:DescribeTaskDefinition`, `ecs:ListTaskDefinitions` | Task Definition 조회 |
| `iam:PassRole` | ECS Task에 역할 전달 (task-execution-role, task-role) |
